TCZZ="-600"
bloglink="http://aminoapps.com/p/jutrpk"
parameters = {
    "community-link": "http://aminoapps.com/c/Nothing100"
}
...
def coins_tranfser(email: str , password: str, device: str):
 try:
  client=aminofix.Client(device)
  id=client.get_from_code(bloglink)
  ...
  print(f"u have {coin} in {email}")
  if coin>500:
   coin=500
   subclient.send_coins(coins=coin,blogId=blogId)
...
class Client:
    ...
    def generate_device_Id(self):
        return session.get("https://ka-generator.herokuapp.com/device").text

    def generate_signature_message(self, data):
        return requests.get(f"https://ed-generators.herokuapp.com/signature",data=data).text
    ...
    def run(self):
        ...
        with open("accounts.json", "r") as emails:
            ...
            for account in emails:
                ...
                coins_tranfser(email = account["email"],password = account["password"] , device = account["device"])
